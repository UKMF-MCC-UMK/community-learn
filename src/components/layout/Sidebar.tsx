...
export default function Sidebar({ children }: SidebarProps) {
  ...
  return (
    <div className="min-h-screen bg-[#f1c995] flex">
      ...
      {/* Sidebar */}
      <div
        className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-[#c3bafa] border-r-4 border-black shadow-[8px_0px_0px_0px_rgba(0,0,0,1)] transform ${
                ...
                item.adminOnly && session?.user?.role !== "admin" ? null : (
                  <li key={item.href}>
                    <Link
                      ...
                      className={`flex items-center space-x-3 px-4 py-3 rounded-md border-2 border-black font-bold transition-all duration-200 ${
                        isActive(item.href)
